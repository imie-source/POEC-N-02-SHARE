...
$().ready(function(){
		...
		var dialogDiv;
		$('#addPersonne').button().on('click',function(){
			console.log('click');
			$.ajax({
				type:'GET',
				url:'./NewPersonneControleur',
				context: $('#dynamicForm')
			}).done(function(retourHttp) {
				$(this).empty();
				$(this).append(retourHttp);
				dialogDiv.dialog('open');
			});
		});
		dialogDiv = $("#dynamicForm").dialog({
			 autoOpen: false,
			 height: 300,
			 width: 350,
			 modal: true
...
<body>
	...
	<button id="addPersonne" >Ajouter une personne</button>
